...
- `DictionaryManager` — loads dictionary data from `assets/dictionary/` on init
- `DictionaryActivity` — search interface for words, idioms, and 成语
...
- `WrongQuestionsActivity` — wrong question review
...
- Platform: Direct APK install (Android)
- Build command: `./gradlew assembleDebug`
- Install command: `adb install -r app/build/outputs/apk/debug/app-debug.apk`
- Or simply: `./gradlew installDebug` (builds + installs in one step)
- Deploy workflow: manual — no CI/CD pipeline
- Merge method: N/A (direct commit to master)
- Project type: Android app
- Health check: N/A (local app, no remote endpoint)
- Post-deploy verification: launch app on device, verify UI loads
